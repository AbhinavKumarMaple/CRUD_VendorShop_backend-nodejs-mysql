...
const registerAdminUser = (req, res) => {
  ...
  connection.query(stateQuery, stateValues, (err, stateResult) => {
    if (err) {
      ...
      connection.query(addressQuery, addressValues, (err, addressResult) => {
        if (err) {
          ...
          // Create the user and link it to the address
          const userQuery =
          ...
          connection.query(userQuery, userValues, (err, userResult) => {
            if (err) {
              ...
              // Get the created admin user's information
              const userInfoQuery = "SELECT * FROM users WHERE id = ?";
              connection.query(userInfoQuery, userId, (err, userInfoResult) => {
                if (err) {
                  ...
                  const adminUser = userInfoResult[0];
                  res.status(200).json({
...
const updateAdminUser = (req, res) => {
  ...
  // Update the state
  const stateQuery =
    "UPDATE states SET state_name = ? WHERE id = (SELECT state_id FROM addresses WHERE id = (SELECT address_id FROM users WHERE id = ?))";
  const stateValues = [StateName, UserId];
  ...
  connection.query(stateQuery, stateValues, (err, stateResult) => {
    if (err) {
      ...
      // Update the address
      const addressQuery =
        "UPDATE addresses SET address_1 = ?, address_2 = ?, pincode = ? WHERE id = (SELECT address_id FROM users WHERE id = ?)";
      const addressValues = [Address1, Address2, Pincode, UserId];
      ...
      connection.query(addressQuery, addressValues, (err, addressResult) => {
        if (err) {
          ...
          // Update the user
          const userQuery =
            "UPDATE users SET email = ?, user_name = ?, pan = ?, user_password = ? WHERE id = ?";
          const hashedPassword = getHashedPassword(Password);
          const userValues = [Email, UserName, PAN, hashedPassword, UserId];
